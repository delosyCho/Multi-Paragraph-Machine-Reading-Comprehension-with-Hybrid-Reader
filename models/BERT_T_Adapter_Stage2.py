...
class KoNET:
    def __init__(self, firstTraining, testCase=False):
        ...
        self.bert_path = '/data/access_paper_models/adapter_pretrain_rtp3/adapter_lm.ckpt'
    ...
    def Training(self, is_Continue, training_epoch):
        ...
        with tf.Session(config=config) as sess:
            ...
            with tf.variable_scope('adapter_layers'):
                ...
                adapter_output, adapter_outputs = self.structured_adapter(table_embeddings,
            ...
            with tf.variable_scope('output_layer_vf'):
                ...
                probs_vf1 = self.get_verify_answer(pooled_output, is_training=True)

            with tf.variable_scope('output_layer_vf_table'):
                pooled_output = model.get_pooled_output()
                pooled_output = tf.concat([pooled_output, tf.squeeze(joint_adapter_output[:, 0:1, :], axis=1)],
                                          axis=-1)

                probs_vf2 = self.get_verify_answer(pooled_output, is_training=True)

            with tf.variable_scope('output_layer_text'):
                sequence_output = tf.concat([adapter_output, sequence_output], axis=-1)
                sequence_output = Fully_Connected(sequence_output, output=768, name='sequence_output', activation=gelu)
                ...
                sequence_output = tf.concat([column_memory,
                                             row_memory,
                                             adapter_output,
                                             joint_adapter_output,
                                             sequence_output], axis=2)
            ...
            vf_loss1 = self.get_vf_loss(probs_vf1) * (tf.ones_like(input_weights) - input_weights)
            vf_loss2 = self.get_vf_loss(probs_vf2) * input_weights

            vf_loss = tf.reduce_mean(vf_loss1) + tf.reduce_mean(vf_loss2)
            ...
            for i in range(training_epoch):
                ...
                input_weights, input_rows, input_cols, vf_label, rank_weight \
                    = self.processor.next_batch_3_batch()
    ...
    def eval_with_span(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            with tf.variable_scope('adapter_layers'):
                ...
                adapter_output, adapter_outputs = self.structured_adapter(table_embeddings,
                                                                          model.all_encoder_layers,
                                                                          return_layer_outputs=True, is_training=False)

            with tf.variable_scope('joint_layers'):
                joint_adapter_output = self.joint_adapter(model.all_encoder_layers, adapter_outputs, is_training=False)
    ...
    def eval_with_ranked_file(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            with tf.variable_scope('output_layer_vf'):
                pooled_output = model.get_pooled_output()
                prob_vf1 = self.get_verify_answer(pooled_output, is_training=False)
            ...
            with tf.variable_scope('output_layer_vf_table'):
                ...
                prob_vf2 = self.get_verify_answer(pooled_output, is_training=False)

            with tf.variable_scope('output_layer_text'):
                sequence_output = tf.concat([adapter_output, sequence_output], axis=-1)
                sequence_output = Fully_Connected(sequence_output, output=768, name='sequence_output', activation=gelu)

                column_memory, row_memory = self.Table_Memory_Network(sequence_output, hops=2, hidden_size=768, dropout=0.0)
            ...
            prob_vf1 = tf.nn.softmax(prob_vf1, axis=-1)
            prob_vf2 = tf.nn.softmax(prob_vf2, axis=-1)
            ...
            for i in range(all_input_ids.shape[0]):
                ...
                probs_start, probs_stop, probs_vf1, probs_vf2, probs_start2, probs_stop2 = \
                    sess.run([prob_start, prob_stop, prob_vf1, prob_vf2, prob_start2, prob_stop2],
                             feed_dict=feed_dict)

                probs_start = np.array(probs_start, dtype=np.float32)
                probs_stop = np.array(probs_stop, dtype=np.float32)

                probs_vf = np.array(probs_vf1, dtype=np.float32)[:, 0]
                probs_vf2 = np.array(probs_vf2, dtype=np.float32)[:, 0]
