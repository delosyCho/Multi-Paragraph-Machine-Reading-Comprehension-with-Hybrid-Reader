...
class KoNET:
    def __init__(self, firstTraining, testCase=False):
        ...
        self.save_path = '/data/access_paper_models/tapas_combined/tapas_kobigbird.ckpt'
    ...
    def eval_with_ranked_file(self):
        #table_model = TAPAS_with_TMN2.KoNET(firstTraining=True)
        #table_model.model_setting()
            ...
            for i in range(all_input_ids.shape[0]):
                ...
                if len(answers) > 0:
                    ...
                    if answer_source_type == 'table':
                        ...
                        f1_total2 += max(a2)
                        epo2 += 1
                    print('answer source type:', answer_source_type)
                    for j in range(input_ids.shape[0]):
                ...
                for article in data['data']:
                    ...
                    for qas in article['qas']:
                        ...
                        if True:
                            ...
                            input_cols = np.zeros(shape=[len(sequences), max_length], dtype=np.int32)
                            input_rows = np.zeros(shape=[len(sequences), max_length], dtype=np.int32)
