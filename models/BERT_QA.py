...
class KoNET:
    def __init__(self, firstTraining, testCase=False):
        ...
        self.save_path = '/data/access_paper_models/vanila_bert_combined/qa_model.ckpt'
    ...
    def create_model(self, input_ids, input_segments, is_training=True, reuse=False, scope='bert'):
    ...
    def Training(self, is_Continue, training_epoch):
        ...
        with tf.Session(config=config) as sess:
            ...
            loss = tf.reduce_mean(loss * self.input_weights)
            vf_loss = tf.reduce_mean(vf_loss)

            total_loss = loss + vf_loss
            ...
            for i in range(training_epoch):
                ...
                _, input_rows, input_cols, vf_label, input_weights\
                ...
                loss_, loss2_, loss3_, _ = sess.run([total_loss, loss, vf_loss, optimizer], feed_dict=feed_dict)
                print(i, loss_, loss2_, loss3_)
    ...
    def model_setting(self, save_path):
        ...
        prob_start, prob_stop = self.get_qa_probs(sequence_output, scope='reader_prediction_layer', is_training=False)
        prob_start = tf.nn.softmax(prob_start, axis=-1)
        prob_stop = tf.nn.softmax(prob_stop, axis=-1)

        prob_vf = self.get_verify_answer(pooled_output, is_training=False, scope='reader_vf_layer')
        prob_vf = tf.nn.softmax(prob_vf, axis=-1)

        self.prob_start = prob_start
        self.prob_stop = prob_stop
        self.prob_vf = prob_vf
    ...
    def propagate(self, input_ids, input_segments):
        feed_dict = {self.input_ids: input_ids,
                     self.input_segments: input_segments}

        probs_start, probs_stop, vf_probs = \
        ...
        return probs_start, probs_stop, vf_probs
    ...
    def Training2(self, is_Continue, training_epoch):
        ...
        with tf.Session(config=config) as sess:
            ...
            optimizer = optimization.create_optimizer(loss=total_loss, init_lr=learning_rate, num_train_steps=training_epoch,
            ...
            for i in range(training_epoch):
                ...
                feed_dict = {self.input_ids: input_ids,
                             ...
                             self.input_rows: input_rows, self.input_cols: input_cols,
                             self.rank_label: vf_label,
                             self.input_weights: rank_weight
                             }
    ...
    def eval_with_ranked_file(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(all_input_ids.shape[0]):
                ...
                probs_start, probs_stop, probs_vf = sess.run([prob_start, prob_stop, prob_vf], feed_dict=feed_dict)
    ...
    def eval_with_file(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            prob_start, prob_stop = self.get_qa_probs(sequence_output,
                                                      scope='reader_prediction_layer',
                                                      is_training=False)
    ...
    def eval_with_file2(self):
        tagger = Mecab()
        chuncker = Chuncker.Chuncker()
        path_dir = '/data/korquad_data/korquad2_dev2/'
