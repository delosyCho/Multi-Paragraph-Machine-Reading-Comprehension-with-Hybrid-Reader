...
class Attention(nn.Module):
    def __init__(self, hidden_size=768, num_heads=16, dropout_prob=0.2):
    ...
    def forward(self, hidden_states, attention_mask):
        # print('device:', hidden_states.device, attention_mask.device)
        ...
        # print('shape1:', attention_scores.shape, 'shape2:', attention_mask.shape)
        ...
        # print('new:', context_layer.shape, new_context_layer_shape)
...
class PositionWiseFeedForward(nn.Module):
    def __init__(self, hidden_size=768, ff_dim=3072, dropout_prob=0.2):
        super(PositionWiseFeedForward, self).__init__()
        self.fc1 = Linear(hidden_size, ff_dim)
        self.fc2 = Linear(ff_dim, hidden_size)
        self.act_fn = gelu
        self.dropout = Dropout(dropout_prob)
...
class Block(nn.Module):
    def __init__(self, hidden_size, ff_dim, dropout_prob):
        super(Block, self).__init__()
        self.attention_norm = LayerNorm(hidden_size, eps=1e-12)
        self.ffn_norm = LayerNorm(hidden_size, eps=1e-12)
        self.ffn = PositionWiseFeedForward(hidden_size=hidden_size, ff_dim=ff_dim, dropout_prob=dropout_prob)
        self.attn = Attention(hidden_size=hidden_size)

    def forward(self, x, attention_mask):
        # Attention
        # print('hidden size:', x.shape, attention_mask.shape)
...
class QuestionAnswering(nn.Module):
    def __init__(self, config):
        ...
        self.qa_hidden1 = Linear(256, 512)
        self.qa_hidden2 = Linear(512, 768)

        self.qa_outputs1 = Linear(768, 512)
        self.qa_outputs2 = Linear(512, 256)
        self.qa_outputs3 = Linear(768, 2)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None,
        ...
        sequence_output = self.qa_hidden1(sequence_output3)
        sequence_output = self.qa_hidden2(sequence_output)

        encoded_layers = self.additional_layer1(sequence_output, extended_attention_mask)
        sequence_output1 = encoded_layers[-1]

        logits1 = self.qa_outputs3(sequence_output1)

        start_logits, end_logits = logits1.split(1, dim=-1)
