...
class KoNET:
    def __init__(self, firstTraining, testCase=False):
        ...
        self.column_mask = tf.placeholder(shape=[None, None], dtype=tf.int32)
        self.row_mask = tf.placeholder(shape=[None, None], dtype=tf.int32)
    ...
    def model_setting(self):
        ...
        probs_start, probs_stop = self.get_qa_probs(sequence_output, is_training=False)

        vars1 = get_variables_with_name('MRC_block', True, True)
        vars2 = get_variables_with_name('pointer_net1', True, True)
    ...
    def Training(self, is_Continue, training_epoch):
        ...
        with tf.Session(config=config) as sess:
            ...
            qa_loss, loss2, loss3 = self.get_qa_loss(probs_start, probs_stop)
            ...
            for i in range(training_epoch):
                ...
                loss_, loss_2, loss_3, _ = sess.run([total_loss, loss2, loss3, optimizer], feed_dict=feed_dict)
                #print(pred[0, 0:10])
                #print(pred2[0, 0:10])
                #print(column_label[0, 0:10])
                print(i, loss_, loss_2, loss_3)
                #print('-------')
                            ...
                            for sub_paragraph in sub_paragraphs:
                                ...
                                for table_text in table_list:
                                    ...
                                    if table_text.find('[answer]') != -1:
                                        ...
                                        rows_max = rows_has_ans.max(axis=1)
                                        cols_max = cols_has_ans.max(axis=1)

                                        row_mask = np.zeros(shape=[input_ids.shape[0], 100], dtype=np.int32)
                                        col_mask = np.zeros(shape=[input_ids.shape[0], 50], dtype=np.int32)

                                        for k in range(input_ids.shape[0]):
                                            for r in range(rows_max[k]):
                                                try:
                                                    row_mask[k, r] = 1
                                                except:
                                                    None
                                            for c in range(cols_max[k]):
                                                try:
                                                    col_mask[k, c] = 1
                                                except:
                                                    None
                                        ...
                                        feed_dict = {self.input_ids: input_ids, self.input_mask: input_mask,
                                                     ...
                                                     self.row_mask: row_mask, self.column_mask: col_mask}
